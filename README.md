...
- **Live stats** — elapsed time, total carbs, and carbs/hour, updated in real time. If you're tracking caffeine and/or sodium, their totals show right alongside as their own stat cards — so the display doesn't get busier just because a metric goes unused, but once it's in use it's just as visible as carbs. Entries still missing a carb count are flagged and excluded from the totals until filled in.
