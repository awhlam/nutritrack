...
export const DEFAULT_PRESETS: Preset[] = [
  { id: 'preset-gel', label: 'Energy Gel', carbs: 30, caffeine: 0, sodium: 0, color: '#f59e0b', kind: 'item' },
  { id: 'preset-water', label: 'Water', carbs: 0, caffeine: 0, sodium: 0, color: '#38bdf8', kind: 'drink' },
  { id: 'preset-carb-mix', label: 'Carb Drink Mix', carbs: 50, caffeine: 0, sodium: 0, color: '#3b82f6', kind: 'drink' },
]
...
function normalizePreset(preset: Preset): Preset {
  return {
    ...preset,
    kind: preset.kind ?? 'item',
    caffeine: preset.caffeine ?? 0,
...
function normalizeSession(session: Session): Session {
  ...
  return {
    ...
    entries: session.entries.map((e) => ({ ...e, caffeine: e.caffeine ?? 0, sodium: e.sodium ?? 0 })),
